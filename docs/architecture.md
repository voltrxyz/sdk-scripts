...
4. **Return everything the processor needs.** ATA-creation instructions, sync-native, vault SDK instructions, adapter-specific instructions — all of them, in order, in `instructions`. Token-account setup always includes an idempotent create instruction, which is an onchain no-op when the account exists. Any LUTs the tx will need go in `lookupTableAddresses`; the processor fetches their contents.
5. **RPC reads are allowed,** but only to construct instructions (e.g. fetching adapter state to derive remaining accounts). Use `ctx.rpc`.
