/**
 * spot:earn-deposit — deposit the vault asset into the Jupiter Earn strategy.
 * Derives accounts from PDAs (offline-capable).
 *
 * Run:   pnpm exec tsx examples/src/spot/earn-deposit.ts   (or: pnpm example -- spot:earn-deposit)
...
const example = defineExample("spot:earn-deposit", async (h) => {
  const manager = await h.signer("manager");
  const operation = await buildSpotEarnDepositOperation(h.ctx, {
    manager,
