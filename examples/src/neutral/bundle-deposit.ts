import { buildNeutralBundleDepositOperation } from "@voltr/scripts-neutral";
import {
  requireVaultAddress,
  requireAssetMint,
...
const example = defineExample("neutral:bundle-deposit", async (h) => {
  const manager = await h.signer("manager");
  const operation = await buildNeutralBundleDepositOperation(h.ctx, {
    manager,
